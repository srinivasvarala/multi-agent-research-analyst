...
Retrieves and summarizes relevant passages from earnings call transcripts.
...
SYSTEM_PROMPT = """You are a specialized financial analyst with deep expertise in earnings call transcripts.
Your job is to analyze retrieved passages from quarterly earnings calls and provide a focused,
...
3. Pay close attention to management tone: are they confident, cautious, defensive, or evasive?
4. Distinguish between retrospective commentary (what happened) and forward guidance (what management expects)
5. If the passages don't contain enough information, clearly state what is missing
6. Focus on what's most relevant to the question — be concise but complete
7. Use structured output via the provided tool — never respond in free text
"""
...
EARNINGS_CALL_ANALYSIS_TOOL = {
    "name": "earnings_call_analysis",
    "description": "Structured output of earnings call transcript analysis with citations",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {
            ...
            "key_findings": {
                "type": "array",
                "items": {"type": "string"},
                "description": "3-5 bullet-point key findings from the earnings call transcripts"
            },
            "cited_chunk_indices": {
                "type": "array",
                "items": {"type": "integer"},
            ...
            "management_tone": {
                "type": "string",
                "description": "Assessment of management tone: e.g. 'confident and bullish', 'cautious about macro headwinds', 'defensive on margin questions'"
            },
            "missing_information": {
                "type": "string",
                "description": "What information would improve this answer but wasn't found in the transcripts"
            }
        },
...
class EarningsCallAgent(BaseAgent):
    """
    Retrieves relevant passages from earnings call transcripts using hybrid search,
    then uses Claude to synthesize a grounded answer with citations and management tone assessment.
    """

    name = AgentName.EARNINGS_CALL

    def __init__(self) -> None:
        super().__init__()
        self.settings = get_settings()
        self.searcher = HybridSearcher(self.settings.chroma_collection_earnings)
    ...
    def _run(self, query: ResearchQuery, memory: SharedMemory) -> AgentResult:
        ...
        chunks = self.searcher.search(
            ...
            ticker=query.ticker,
            doc_types=[DocType.EARNINGS_CALL],
        )
        ...
        if not chunks:
            ...
            return AgentResult(
                ...
                chunks=[],
                summary=(
                    f"No earnings call transcripts found for {query.ticker}. "
                    "Please ingest transcripts first using scripts/ingest_transcripts.py"
                ),
                confidence=0.0,
            )
        ...
        raw_response, tokens = self._call_claude(
            ...
            tools=[EARNINGS_CALL_ANALYSIS_TOOL],
...
Retrieved Earnings Call Transcript Passages:
---
{context}
---

Please analyze these passages and answer the research question using the earnings_call_analysis tool.
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test EarningsCallAgent directly")
    ...
    if result.error:
        ...
        console.print(Panel(
            ...
            title=f"[bold]Earnings Call Analysis — {args.ticker.upper()}[/bold]",
            subtitle=f"Confidence: {result.confidence:.0%} | Chunks cited: {len(result.chunks)} | {result.latency_ms}ms",
        ))
        tone = mem.scratchpad.get("earnings_call", {}).get("management_tone")
        if tone:
            console.print(f"\n[bold]Management Tone:[/bold] {tone}")
